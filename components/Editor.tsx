...
import { improveSummary } from '../services/gemini';
...
const Editor: React.FC<EditorProps> = ({ data, onChange }) => {
  ...
  const handleImproveSummary = async () => {
    ...
    setIsImproving(true);
    try {
      const improved = await improveSummary(data.personalInfo.summary, data.personalInfo.jobTitle);
      updatePersonalInfo('summary', improved);
    } catch (error) {
      console.error(error);
      alert("AI 요약 개선에 실패했습니다.");
    } finally {
  ...
  const SectionHeader = ({ icon, title, isVisible, onToggle, onAdd }: { icon: string, title: string, isVisible?: boolean, onToggle?: () => void, onAdd?: () => void }) => (
  ...
  return (
    <div className="flex flex-col gap-8 p-6 pb-24 h-full overflow-y-auto no-print">
      {/* 기본 정보 */}
      <section>
        <SectionHeader icon="fa-solid fa-user" title="기본 정보" />
            ...
            <div className="flex justify-between items-center mb-1">
              <div className="flex items-center gap-2">
                ...
                <button 
                  ...
                  className={`text-[10px] px-1.5 py-0.5 rounded border ${data.visibility.summary ? 'text-blue-500 border-blue-200' : 'text-slate-400 border-slate-200'}`}
                >
                  {data.visibility.summary ? '노출' : '숨김'}
                </button>
              </div>
              <button onClick={handleImproveSummary} disabled={isImproving} className="text-xs flex items-center gap-1 text-blue-600 hover:text-blue-800 transition-colors disabled:opacity-50">
                {isImproving ? <i className="fa-solid fa-spinner fa-spin"></i> : <i className="fa-solid fa-wand-magic-sparkles"></i>} AI 요약 개선
              </button>
            </div>
      ...
      {/* 경력 사항 */}
      <section>
        <SectionHeader icon="fa-solid fa-briefcase" title="경력 사항" isVisible={data.visibility.experiences} onToggle={() => toggleVisibility('experiences')} onAdd={addExperience} />
        <div className={`space-y-6 ${!data.visibility.experiences && 'opacity-50 grayscale pointer-events-none'}`}>
          {data.experiences.map((exp) => (
            <div key={exp.id} className="relative p-5 border rounded-xl bg-white shadow-sm border-slate-200 hover:border-blue-200 transition-colors">
              <button onClick={() => removeExperience(exp.id)} className="absolute top-4 right-4 text-slate-300 hover:text-red-500 transition-colors">
                ...
                <div className="col-span-2 mt-6">
                  <div className="flex justify-between items-center mb-4 pb-2 border-b border-slate-50">
                    <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider flex items-center gap-2">
                      <i className="fa-solid fa-list-check text-blue-400"></i> 주요 성과 및 세부 프로젝트
                    </h4>
                    <button onClick={() => addAchievement(exp.id)} className="text-[11px] bg-blue-50 text-blue-600 px-2.5 py-1 rounded-full hover:bg-blue-100 transition-colors flex items-center gap-1">
                      <i className="fa-solid fa-plus"></i> 항목 추가
                    </button>
                  </div>
                  ...
                  <div className="space-y-5">
                    {exp.achievements.map((a) => (
                      <div key={a.id} className="group relative bg-slate-50 p-4 rounded-lg border border-slate-100">
                        <button 
                          onClick={() => removeAchievement(exp.id, a.id)}
                          className="absolute -top-2 -right-2 w-6 h-6 bg-white border border-slate-200 text-slate-400 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 hover:text-red-500 transition-all shadow-sm"
                        >
                        ...
                        <div className="grid grid-cols-3 gap-3 mb-3">
                          <div className="col-span-3 sm:col-span-1">
                            ...
                            <input 
                              type="text" 
                              value={a.title} 
                              onChange={(e) => updateAchievement(exp.id, a.id, 'title', e.target.value)}
                              className="w-full text-sm font-semibold bg-white border border-slate-200 rounded px-2 py-1 outline-none focus:border-blue-300"
                              placeholder="프로젝트/성과 제목"
                            />
                          </div>
                          <div className="col-span-3 sm:col-span-1">
                            ...
                            <input 
                              type="text" 
                              value={a.period} 
                              onChange={(e) => updateAchievement(exp.id, a.id, 'period', e.target.value)}
                              className="w-full text-sm bg-white border border-slate-200 rounded px-2 py-1 outline-none focus:border-blue-300"
                          ...
                          <div className="col-span-3 sm:col-span-1">
                            ...
                            <input 
                              type="text" 
                              value={a.role} 
                              onChange={(e) => updateAchievement(exp.id, a.id, 'role', e.target.value)}
                              className="w-full text-sm bg-white border border-slate-200 rounded px-2 py-1 outline-none focus:border-blue-300"
                              placeholder="수행 역할"
                            />
                          </div>
                        ...
                        <div>
                          ...
                          <textarea 
                            rows={2}
                            value={a.content}
                            onChange={(e) => updateAchievement(exp.id, a.id, 'content', e.target.value)}
                            className="w-full text-sm bg-white border border-slate-200 rounded px-2 py-1 outline-none focus:border-blue-300 resize-none"
                            placeholder="상세 내용을 입력하세요 (여러 줄 가능)"
                          />
                        </div>
                      </div>
                    ))}
      ...
      {/* 학력 사항 */}
      <section>
        <SectionHeader icon="fa-solid fa-graduation-cap" title="학력 사항" isVisible={data.visibility.educations} onToggle={() => toggleVisibility('educations')} onAdd={addEducation} />
        <div className={`space-y-6 ${!data.visibility.educations && 'opacity-50 grayscale pointer-events-none'}`}>
          {data.educations.map((edu) => (
            <div key={edu.id} className="relative p-4 border rounded-lg bg-white shadow-sm border-slate-200">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-1">
                  <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider">학교명</label>
                  <input type="text" value={edu.school} onChange={(e) => updateEducation(edu.id, 'school', e.target.value)} className="mt-1 block w-full border-b focus:border-blue-500 sm:text-sm outline-none p-1" />
                </div>
                <div className="col-span-1">
                  <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider">전공</label>
                  <input type="text" value={edu.major} onChange={(e) => updateEducation(edu.id, 'major', e.target.value)} className="mt-1 block w-full border-b focus:border-blue-500 sm:text-sm outline-none p-1" />
                </div>
                <div className="col-span-1">
                  <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider">학위</label>
                  <input type="text" value={edu.degree} onChange={(e) => updateEducation(edu.id, 'degree', e.target.value)} className="mt-1 block w-full border-b focus:border-blue-500 sm:text-sm outline-none p-1" />
                </div>
                <div className="col-span-1">
                  <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider">졸업년도</label>
                  <input type="text" value={edu.endDate} placeholder="YYYY-MM" onChange={(e) => updateEducation(edu.id, 'endDate', e.target.value)} className="mt-1 block w-full border-b focus:border-blue-500 sm:text-sm outline-none p-1" />
                </div>
              </div>
            </div>
          ))}
      ...
      {/* 보유 기술 */}
      <section>
        <SectionHeader icon="fa-solid fa-screwdriver-wrench" title="보유 기술" isVisible={data.visibility.skills} onToggle={() => toggleVisibility('skills')} onAdd={addSkill} />
          ...
          {data.skills.map((skill) => (
            <div key={skill.id} className="flex items-center gap-3 p-2 border rounded-md bg-slate-50 border-slate-200">
              <input 
                ...
                className="bg-transparent text-sm font-medium w-full outline-none focus:border-b border-blue-400" 
                placeholder="스킬명 (예: React, SQL, Photoshop)" 
              />
              <button onClick={() => removeSkill(skill.id)} className="text-slate-400 hover:text-red-500 px-1">
                <i className="fa-solid fa-xmark"></i>
              </button>
            </div>
          ))}
          {data.skills.length === 0 && (
            <div className="col-span-2 text-center py-4 bg-slate-50 border border-dashed border-slate-200 rounded-lg">
              <p className="text-xs text-slate-400">보유하고 있는 기술이나 역량을 추가해보세요.</p>
            </div>
          )}
        </div>
      </section>
      ...
      {/* 커스텀 섹션 */}
      <section>
        <SectionHeader icon="fa-solid fa-plus" title="커스텀 섹션" onAdd={addCustomSection} />
        <div className="space-y-8">
          {data.customSections.map((section) => (
            <div key={section.id} className="relative p-5 border rounded-xl bg-slate-50 border-slate-200">
              <div className="flex justify-between items-center mb-6">
                <input 
                  type="text" 
                  value={section.title} 
                  onChange={(e) => updateCustomSection(section.id, 'title', e.target.value)} 
                  className="bg-white px-3 py-1.5 rounded-lg border border-slate-200 font-bold text-slate-800 outline-none focus:border-blue-400 shadow-sm"
                  placeholder="섹션 제목 (예: 자격증, 어학능력)"
                />
                <div className="flex items-center gap-2">
                  <button 
                    onClick={() => updateCustomSection(section.id, 'visible', !section.visible)}
                    className={`text-xs px-2.5 py-1.5 rounded-full border transition-all ${section.visible ? 'bg-blue-50 border-blue-200 text-blue-600' : 'bg-white border-slate-200 text-slate-400'}`}
                  >
                    {section.visible ? <i className="fa-solid fa-eye mr-1"></i> : <i className="fa-solid fa-eye-slash mr-1"></i>}
                    {section.visible ? '노출 중' : '비노출'}
                  </button>
                  <button onClick={() => removeCustomSection(section.id)} className="w-8 h-8 rounded-full flex items-center justify-center text-slate-400 hover:bg-white hover:text-red-500 transition-all border border-transparent hover:border-slate-100">
                    <i className="fa-solid fa-trash-can"></i>
                  </button>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <button onClick={() => addCustomItem(section.id)} className="text-[10px] bg-white text-blue-600 px-2 py-1 rounded border border-blue-100 hover:bg-blue-50 transition-colors">
                    + 항목 추가
                  </button>
                </div>
                  ...
                  {section.items.map((item) => (
                    <div key={item.id} className="relative bg-white p-4 rounded-lg border border-slate-100 shadow-sm group">
                      <button 
                        ...
                        className="absolute -top-2 -right-2 w-6 h-6 bg-white border border-slate-200 text-slate-300 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 hover:text-red-500 transition-all shadow-sm"
                      >
                        <i className="fa-solid fa-xmark text-[10px]"></i>
                      </button>
                      ...
                      <div className="grid grid-cols-2 gap-3 mb-3">
                        <div className="col-span-2 sm:col-span-1">
                          ...
                          <input 
                            ...
                            className="w-full text-sm font-semibold border-b border-slate-100 focus:border-blue-300 outline-none pb-0.5"
                            placeholder="항목 제목"
                          />
                        </div>
                        <div className="col-span-2 sm:col-span-1">
                          ...
                          <input 
                            type="text" 
                            value={item.period} 
                            onChange={(e) => updateCustomItem(section.id, item.id, 'period', e.target.value)}
                            className="w-full text-sm border-b border-slate-100 focus:border-blue-300 outline-none pb-0.5"
                            placeholder="YYYY.MM"
                          />
                        </div>
                        <div className="col-span-2">
                          <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">부제 / 소속</label>
                          <input 
                            type="text" 
                            value={item.subtitle} 
                            onChange={(e) => updateCustomItem(section.id, item.id, 'subtitle', e.target.value)}
                            className="w-full text-sm border-b border-slate-100 focus:border-blue-300 outline-none pb-0.5"
                            placeholder="부제목 또는 발급 기관"
                          />
                        </div>
                      </div>
                      <div>
                        ...
                        <textarea 
                          rows={2}
                          value={item.content}
                          onChange={(e) => updateCustomItem(section.id, item.id, 'content', e.target.value)}
                          className="w-full text-sm border-b border-slate-100 focus:border-blue-300 outline-none pb-0.5 resize-none"
      ...
      {/* 테마 설정 */}
      <section>
        <SectionHeader icon="fa-solid fa-palette" title="테마 설정" />
        <div className="flex gap-4">
          {(['classic', 'modern', 'minimal'] as const).map((t) => (
            <button key={t} onClick={() => onChange({ ...data, theme: t })} className={`px-4 py-2 rounded-lg border-2 transition-all capitalize ${data.theme === t ? 'border-blue-500 bg-blue-50 text-blue-600 font-bold' : 'border-slate-200 text-slate-500 hover:border-slate-300'}`}>{t}</button>
          ))}
        </div>
        <div className="mt-4">
          <label className="block text-sm font-medium text-gray-700 mb-2">포인트 컬러</label>
          <div className="flex gap-2">
            {['#2563eb', '#dc2626', '#16a34a', '#7c3aed', '#ea580c', '#334155'].map((color) => (
              <button key={color} onClick={() => onChange({ ...data, primaryColor: color })} style={{ backgroundColor: color }} className={`w-8 h-8 rounded-full border-2 ${data.primaryColor === color ? 'border-white ring-2 ring-slate-300' : 'border-transparent'}`} />
            ))}
          </div>
        </div>
      </section>
